...
;; Events are generated by the above functions using https://www.alien.net.au/irc/irc2numerics.html as a template.
;; All events need to be manually checked and adapted for names and proper arguments lists though.
;;
...
(define-event myinfo-event :RPL_MYINFO (event)
    (server-name version user-modes chan-modes &optional channel-modes-with-params user-modes-with-params server-modes server-modes-with-params)
...
(define-event trace-link-event :RPL_TRACELINK (event)
    (link version destination next-server &optional vprotocol-version link-uptime-in-seconds backstream-sendq upstream-sendq)
...
(define-event trace-unknown-event :RPL_TRACEUNKNOWN (event)
    (NIL group &optional connection-address)
    (:documentation "See RFC"))
...
(define-event stats-iline-event :RPL_STATSILINE (event)
    (NIL host NIL host2 port group)
    (:documentation "Reply to STATS (See RFC)"))

(define-event stats-kline-event :RPL_STATSKLINE (event)
    (NIL host NIL username port group)
    (:documentation "Reply to STATS (See RFC)"))

(define-event stats-yline-event :RPL_STATSYLINE (event)
    (NIL group ping-freq connect-freq max-sendq)
    (:documentation "Reply to STATS (See RFC)"))
...
(define-event umode-event :RPL_UMODEIS (event)
    (user-modes &optional user-mode-params)
...
(define-event namreply-event :RPL_NAMREPLY (event)
    (name-type channel &rest nickinfo)
...
(define-event banlist-event :RPL_BANLIST (event)
    (channel banid &optional time-left reason )
...
(define-event traceroute-hop-event :RPL_TRACEROUTE_HOP (event)
    (target hop &optional address hostname usec-ping)
...
(define-event whois-secure-event :RPL_WHOISSECURE (event)
    (nick who-type &optional message)
...
(define-event time-on-server-event :RPL_TIMEONSERVERIS (event)
    (seconds nanoseconds timezone flags message)
...
(define-event networks-event :RPL_NETWORKS (event)
    (name through-name hops message)
...
(define-event language-event :RPL_LANGUAGE (event)
    (code revision maintainer flags NIL message)
